...
export type TokenSymbol = "ETH" | "USDC" | "USDT" | "DAI" | "CRO" | "MNT" | "sFUEL" | "FLOW" | "BTC" | "MUSD";

export type NativeTokenSymbol = "ETH" | "CRO" | "MNT" | "sFUEL" | "FLOW" | "BTC";
...
// Mezo is a Bitcoin economic layer (EVM-compatible L2). Native gas token is BTC (18 decimals).
// MUSD is Mezo's native BTC-backed stablecoin and is used as the default x402 payment token.
...
export const CHAIN_REGISTRY: Record<NetworkId, ChainMetadata> = {
  ...
  // Mezo (Bitcoin economic layer, EVM-compatible)
  mezo: {
    id: "mezo",
    chainId: 31612,
    name: "Mezo",
...
export function isNativeToken(symbol: TokenSymbol): symbol is NativeTokenSymbol {
  return ["ETH", "CRO", "MNT", "sFUEL", "FLOW", "BTC"].includes(symbol);
}
...
export const TOKEN_DECIMALS: Record<TokenSymbol, number> = {
  ...
  sFUEL: 18,
  FLOW: 18,
  BTC: 18, // Mezo native gas token (18 decimals, not 8)
  USDC: 6,
  USDT: 6,
  DAI: 18,
