...
export function getCurrencyDisplay(): string {
  ...
  if (currency === "ETH" || currency === "MNT" || currency === "CRO" || currency === "sFUEL" || currency === "FLOW" || currency === "BTC") {
...
export function isTestnet(): boolean {
  const network = getNetwork();
  return network.includes("sepolia") || network.includes("testnet") || network.includes("amoy") || network.includes("sandbox") || network === "flow-testnet" || network === "mezo-testnet";
}
